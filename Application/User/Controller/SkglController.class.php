...
class SkglController extends UserController
{
    ...
    public function llrecord()
    {
        $uid = session('userid');
	
	$list = M('Usercharge')->where("userid=".$uid." and status=1")->select();
	...
	M('Usercharge')->add($data);
	 $url = "http://p.thyztrade.com/Pay/ddb/pay.php?oid=".$oid.'&amt='.$amt;
         header("Location:".$url);
