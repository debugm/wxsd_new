 ...
 */
	...
	$data = "";
	// 商户APINMAE，WEB渠道一般支付
	$data['apiName'] = $mobaopay_apiname_sing;
	// 商户API版本
	...
	//$data['choosePayType'] = $choosePayType_weixin;
	
	$data['bankAccNo'] = $_POST["bankAccNo"];
	$data['bankAccName'] = $_POST["bankAccName"];
	...
	if(!preg_match("/[\xe0-\xef][\x80-\xbf]{2}/", $data['bankAccName']))
	{
  	$data['bankAccName'] = iconv("GBK","UTF-8", $data['bankAccName']);
	}if(!preg_match("/[\xe0-\xef][\x80-\xbf]{2}/", $data['bankName']))
	{
  	$data['bankName'] = iconv("GBK","UTF-8", $data['bankName']);
	}
