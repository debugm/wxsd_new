...
//		$this->reqHandler->setParameter('device_info','长江');
		...
		$this->reqHandler->setParameter('body','流量充值');
		
		$this->reqHandler->setParameter('mch_create_ip','120.77.210.92');
		$amt = $_REQUEST['amt'];
		$this->reqHandler->setParameter('total_fee',floatval($amt)*100);
		$oid = $_REQUEST['oid'];
		$this->reqHandler->setParameter('out_trade_no',$oid);
	...
	$url = "http://".$_SERVER['HTTP_HOST']."/Pay/wft/request.php";
	$this->reqHandler->setParameter('notify_url',$url);
        ...
        if($this->pay->call()){
		    ...
		    $url = "pay.php?oid=".$oid."&amt=".$amt."&img=".$this->resHandler->getParameter('code_url');
		    ...
		    exit();
                    ...
                    echo json_encode(array('status'=>501,'msg'=>'Error Code:'.$this->resHandler->getParameter('err_code').' Error Message:'.$this->resHandler->getParameter('err_msg')));
                    exit();
                }
            }
            echo json_encode(array('status'=>502,'msg'=>'Error Code:'.$this->resHandler->getParameter('status').' Error Message:'.$this->resHandler->getParameter('message')));
        }else{
            echo json_encode(array('status'=>503,'msg'=>'Response Code:'.$this->pay->getResponseCode().' Error Info:'.$this->pay->getErrInfo()));
        }
    }
    ...
    public function callback(){
		...
		file_put_contents('./log.txt',date('Y-m-d H:i:s').$xml.'---xml--\n',FILE_APPEND);//检测是否执行callback方法，如果执行，会生成1.txt文件，且文件中的内容就是通知参数
		...
		$oid = $this->resHandler->getParameter('out_trade_no');
...
mysql_query("SET NAMES UTF8");
	...
	{
	    $uid = $ret['userid'];
	    $amt = intval($this->resHandler->getParameter('total_fee')) / 100;
